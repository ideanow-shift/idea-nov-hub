...
Actual can enter only through a validated M012 Batch and a `valid` Staging Line whose normalization and mapping passed and whose Canonical tax basis is `exclusive`. M015 independently revalidates both the Journal Line and Fact boundary: source amount is mandatory and finite; source tax basis/category/rate-source evidence cannot be unknown; normalized amount obeys observed/zero/not-applicable semantics and is finite when present. A rounding mode that performs rounding (`floor`, `ceiling`, `half_up`, `half_even`, `truncate`) requires `line` or `document` scope, a finite positive unit and a finite difference amount. `not_applicable` mode requires `not_applicable` scope; any optional unit is finite positive and any optional difference is NULL or formal zero. Incomplete or non-finite evidence remains quarantined even when M012 status fields were incorrectly marked passed/valid. M012 history is not modified; tightening its quarantine-state transition is a separate future corrective candidate.

M015 additively seals Batch membership: finalizing a Batch serializes against File/Line membership writes, and a validated/rejected/future-terminal Batch rejects File/Line INSERT, UPDATE, DELETE, and movement from or into the sealed Batch. This closes late-line promotion without changing M012 history. Budget/Forecast may use a nonblank versioned planning contract; an imported plan still follows M012. Raw identifiers, raw payload, Production IDs and PII are prohibited.
...
Canonical amounts use finite `numeric(20,4)`, currency `JPY` and `tax_basis='exclusive'`. `NaN` and positive/negative infinity are rejected from Fact and allocation monetary fields and from every Actual source/normalization/rounding amount at promotion. `observed` requires non-NULL/nonzero, `zero` requires exactly zero, and `not_applicable` requires a NULL normalized amount while the source amount still remains explicit and finite. Missing, pending, validation-failed, incomplete-evidence and non-finite-evidence candidates remain in M012 quarantine and cannot become Facts. No numeric column has a semantic default of zero.
