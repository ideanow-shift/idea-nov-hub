...
await test('identity mismatch executes zero queries', async () => {
  const db = new FakeAuditConnection(); const result = await runSealedAudit({ request: request(['C01_TARGET_RELATIONS']), profile, identityObservation: { ...observation, hostMatch: false }, connection: db });
  assert.equal(result.queryCount, 0); assert.deepEqual(db.events, []);
});
await test('non-audit role is rejected before open', async () => {
  const db = new FakeAuditConnection({ roleClass: 'application_role' }); const result = await runSealedAudit({ request: request(['C01_TARGET_RELATIONS']), profile, identityObservation: observation, connection: db });
...
await test('service role and writable identity are rejected', async () => {
  for (const db of [new FakeAuditConnection({ serviceRole: true }), new FakeAuditConnection({ canWrite: true })]) {
    const result = await runSealedAudit({ request: request(['C01_TARGET_RELATIONS']), profile, identityObservation: observation, connection: db }); assert.equal(result.queryCount, 0);
  }
});
...
await test('query count exceeding ten is rejected', async () => {
  const db = new FakeAuditConnection(); const result = await runSealedAudit({ request: request(Array(11).fill('C01_TARGET_RELATIONS').map((id, i) => `${id}_${i}`)), profile, identityObservation: observation, connection: db }); assert.equal(result.queryCount, 0);
});
await test('readonly guard failure rolls back', async () => {
  const db = new FakeAuditConnection({ readOnly: false }); const result = await runSealedAudit({ request: request(['C01_TARGET_RELATIONS']), profile, identityObservation: observation, connection: db }); assert.equal(result.failureCategory, 'READ_ONLY_SESSION_UNVERIFIED'); assert.deepEqual(db.events, ['open', 'begin_read_only', 'verify_read_only', 'rollback', 'close']);
});
await test('fixed query succeeds and always rolls back', async () => {
  const db = new FakeAuditConnection({ rowsByQuery: { C01_TARGET_RELATIONS: [{ schema_name: 'public', relation_name: 'stores', relation_kind: 'r' }] } }); const result = await runSealedAudit({ request: request(['C01_TARGET_RELATIONS']), profile, identityObservation: observation, connection: db }); assert.equal(result.runStatus, 'complete'); assert.equal(result.queryCount, 1); assert.equal(db.events.at(-2), 'rollback'); assert.equal(validateAuditResult(result), true);
});
await test('partial failure rolls back without raw error', async () => {
  const db = new FakeAuditConnection({ rowsByQuery: { C01_TARGET_RELATIONS: [{ schema_name: 'public', relation_name: 'stores', relation_kind: 'r' }] }, failQuery: 'C02_COLUMN_SHAPE' }); const result = await runSealedAudit({ request: request(['C01_TARGET_RELATIONS', 'C02_COLUMN_SHAPE']), profile, identityObservation: observation, connection: db }); assert.equal(result.failureCategory, 'AUDIT_QUERY_FAILED'); assert.equal(db.events.at(-2), 'rollback'); assert.equal(JSON.stringify(result).includes('FAKE_QUERY_FAILURE'), false);
});
...
await test('registry contains exactly ten validated immutable query ids', async () => {
  assert.equal(QUERY_IDS.length, 10); assert.equal(new Set(QUERY_IDS).size, 10); assert.equal(FIXED_QUERY_REGISTRY.every((query) => validateFixedSql(query.sql)), true);
});
