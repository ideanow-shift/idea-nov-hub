...
HUBログイン済みの場合は、HUB Contextを優先して店舗ID/PASS入力をスキップします。Contextがない場合だけPhase1店舗ログインへfallbackします。
...
`レセプション` は役職ではなく職種として扱います。職種条件が必要な場合は `employees.job_type_id -> public.job_types.id` を参照します。`一般スタッフ` は役職候補ですが、管理権限ではありません。
