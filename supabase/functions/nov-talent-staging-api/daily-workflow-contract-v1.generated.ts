...
export type assignee = { "employeeId": string; "displayName": string; };
export type communication = { "id": string; "candidateId": string; "occurredAt": string; "method": "LINE" | "PHONE" | "EMAIL" | "IN_PERSON" | "SCHOOL_RELAY" | "OTHER"; "direction": "INBOUND" | "OUTBOUND"; "result": "REACHED" | "NO_RESPONSE" | "REPLY_RECEIVED" | "INFORMATION_SHARED" | "OTHER"; "summary": string; "awaitingReply": boolean; "nextFollowUpDate": nullableDate; "correctsCommunicationId": nullableUuid; "correctionReason": string | null; "correctionCreatedAt": nullableDateTime; "isCorrection": boolean; "isEffective": boolean; "version": number; };
export type nextAction = { "id": string; "candidateId": string; "code": "FOLLOW_UP" | "SALON_TOUR_FOLLOW_UP" | "INTERVIEW_FOLLOW_UP" | "OFFER_FOLLOW_UP"; "dueDate": nullableDate; "text": nullableString; "assignedTo": nullableString; "assignedEmployeeId": nullableUuid; "assigneeState": "REGISTERED" | "UNREGISTERED"; "isMine": boolean; "state": "OPEN" | "ON_HOLD" | "COMPLETED" | "CANCELLED"; "holdReason": nullableString; "version": number; "creationBasis": "MANUAL" | "COMMUNICATION_FOLLOW_UP"; "originCommunicationId": nullableUuid; };
export type DailyWorkflowResponseV1 = { "ok": true; "data": { "daily_workflow_contract_version": "1.1.0"; "sourceCoverageState": "COMPLETE" | "PREPARING"; "generatedAt": string; "assignees": ReadonlyArray<assignee>; "communications": ReadonlyArray<communication>; "nextActions": ReadonlyArray<nextAction>; }; };

type DailyWorkflowJsonSchema = Record<string, any>;
...
function resolveRef(root: DailyWorkflowJsonSchema, ref: string): DailyWorkflowJsonSchema {
  return String(ref).split("/").slice(1).reduce((value, segment) => value?.[segment.replace(/~1/gu, "/").replace(/~0/gu, "~")], root);
}

function formatValid(value: string, format: string): boolean {
...
function validateNode(value: any, node: DailyWorkflowJsonSchema, root: DailyWorkflowJsonSchema, pathName: string): DailyWorkflowValidationResult {
  if (node.$ref) return validateNode(value, resolveRef(root, node.$ref), root, pathName);
  for (const child of node.allOf || []) { const result = validateNode(value, child as DailyWorkflowJsonSchema, root, pathName); if (!result.ok) return result; }
  if (node.if) {
  ...
  if (node.type === "object") {
    ...
    for (const [key, child] of Object.entries(node.properties || {})) if (Object.hasOwn(value, key)) { const result = validateNode(value[key], child as DailyWorkflowJsonSchema, root, pathName + "." + key); if (!result.ok) return result; }
  }
  if (node.type === "array") {
...
export function validateDailyWorkflowResponse(value: any): DailyWorkflowResponseValidationResult {
  const result = validateNode(value, SCHEMA, SCHEMA, "dailyWorkflow");
  return result.ok ? { ok: true, value: value as DailyWorkflowResponseV1 } : result;
}
