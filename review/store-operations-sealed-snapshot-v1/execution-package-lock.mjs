...
export function verifyExecutionPackage({ packageRoot = PACKAGE_ROOT } = {}) {
  ...
  const exactKeys = ['lockVersion', 'packageId', 'packageVersion', 'artifacts', 'queryPackSha256', 'schemaContractSha256', 'sanitizerContractSha256', 'manifestContractSha256', 'packageSha256'];
  if (!supplied || typeof supplied !== 'object' || Array.isArray(supplied)
    ...
    || ['queryPackSha256', 'schemaContractSha256', 'sanitizerContractSha256', 'manifestContractSha256'].some((key) => !HASH.test(supplied[key] ?? ''))) {
    reject('PACKAGE_INTEGRITY_REJECTED');
  }
