...
const enabled=process.env.BDF_RUN_M018_DB_REHEARSAL==='1';
...
test('M001-M018 plus M061-M063 are deterministic with published Projection regression',{
  skip:!enabled&&'set BDF_RUN_M018_DB_REHEARSAL=1',timeout:900000
},async()=>{
  assert.ok(pgBin);
  const dir=await mkdtemp(path.join(os.tmpdir(),'bdf-m018-pg17-'));
  ...
  try{
    ...
    await apply();
    await psql(await file('supabase/validation/pr002/validate_m018.sql'));
    ...
    assert.equal(psql("select count(*) from pg_views where schemaname='projection' and viewname like 'accounting_%_v1';").stdout.trim(),'0');
