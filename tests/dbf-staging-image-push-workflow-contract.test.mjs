...
assert.match(workflow, /existing_tag_count=.*images-before\.json/u);
assert.match(workflow, /test "\$existing_tag_count" = "0"/u);
assert.doesNotMatch(workflow, /existing-tag\.err|NOT_FOUND\|not found\|does not exist/u);
assert.match(workflow, /serving_revision=.*traffic\[0\]\.revisionName/u);
