...
import { assertExecutionInterfaces, assertReadOnlyConnection } from './broker-interface.mjs';
...
const TARGET_PRESTATE_ZERO_FIELDS = Object.freeze([
  'canonical_corporation_count',
  'canonical_store_count',
  'canonical_employee_count',
  'canonical_role_count',
  'canonical_assignment_count',
  'identity_crosswalk_count',
  'consumer_anchor_count',
  'consumer_access_contract_count',
  'partial_population_count',
  'duplicate_count',
  'orphan_count',
]);
...
const READ_ONLY_EVIDENCE_COUNTS = Object.freeze([
  ...
  'noinherit_violation_count',
  'connection_limit_violation_count',
  'valid_until_violation_count',
  ...
  'direct_application_routine_execute_count',
...
function assertExpectedProfile(profile, environment) {
  const fields = ['profileReference', 'profileFingerprint', 'environment', 'projectIdentityReference', 'brokerReference', 'expectedSnapshotRole', 'notBefore', 'expiresAt', 'postgresVersionPolicy'];
  if (!profile || typeof profile !== 'object' || fields.some((field) => !Object.hasOwn(profile, field))
    ...
    || !/^[a-z][a-z0-9_]{2,62}$/.test(profile.expectedSnapshotRole ?? '')
...
function assertReadOnly(attestation) {
  try { assertRoleExecutionContainment(attestation); } catch { failure('READ_ONLY_ROLE_REJECTED'); }
}
...
function assertReadOnlyEvidence(row, side, schemaContract) {
  ...
  for (const key of ['reachable_role_count', 'settable_role_count', 'inherited_role_count', 'login_role_count', 'application_schema_count', ...READ_ONLY_EVIDENCE_COUNTS]) {
  ...
  for (const key of ['reachable_role_count', 'settable_role_count', 'inherited_role_count', 'login_role_count']) requireValue(row, key, 1, 'READ_ONLY_ROLE_REJECTED');
  for (const key of ['effective_temp_privilege_count', 'executable_application_routine_count']) {
    if (!Number.isSafeInteger(row[key]) || row[key] < 0) failure('READ_ONLY_ROLE_REJECTED');
  }
...
function assertDomainGates(stage1Records) {
  ...
  if (employees.sales_department_head_state === 'resolved'
    && (employees.sales_department_head_candidate_count !== 1
      || typeof employees.sales_department_head_employee_key !== 'string'
      || employees.sales_department_head_employee_key.length === 0
      || typeof employees.sales_department_head_employee_number !== 'string'
      || employees.sales_department_head_employee_number.length === 0)) failure('DOMAIN_VALIDATION_REJECTED');
  ...
  if (Object.keys(target).length !== TARGET_PRESTATE_ZERO_FIELDS.length
    || TARGET_PRESTATE_ZERO_FIELDS.some((key) => !Object.hasOwn(target, key))) failure('DOMAIN_VALIDATION_REJECTED');
  for (const key of TARGET_PRESTATE_ZERO_FIELDS) requireValue(target, key, 0);
...
async function executePacks({ packIds, sourceConnection, targetConnection, sourceRole, targetRole, result, evidence, resources, packageRoot }) {
  const records = [];
  for (const packId of packIds) {
    for (const query of getQueriesForPack(packId)) {
      ...
      let securityBinding;
      try { securityBinding = assertQuerySecurity(query, sqlArtifact.sqlText); } catch (error) { failure(error.code ?? 'QUERY_ALLOWLIST_REJECTED', query.queryId); }
      const connection = query.side === 'source' ? sourceConnection : targetConnection;
      const expectedRole = query.side === 'source' ? sourceRole : targetRole;
      let rows;
      try {
        const runtimeEvidence = await connection.attestRuntimeEvidence({
          expectedRole,
          queryId: query.queryId,
          querySha256: sqlArtifact.sqlSha256,
          astSha256: securityBinding.astSha256,
          queryOrdinal: result.queryCount + 1,
          expectedQueryCount: FIXED_QUERY_REGISTRY.length,
        });
        assertRuntimeEvidence(runtimeEvidence, { expectedRole, query, queryOrdinal: result.queryCount + 1, expectedQueryCount: FIXED_QUERY_REGISTRY.length });
        rows = await connection.executeFixedQuery({
          packageId: PACKAGE_ID,
          packageVersion: PACKAGE_VERSION,
          ...
          sqlSha256: sqlArtifact.sqlSha256,
          astSha256: securityBinding.astSha256,
          queryOrdinal: result.queryCount + 1,
          expectedQueryCount: FIXED_QUERY_REGISTRY.length,
        ...
        const allowed = ['FIXED_SQL_HASH_MISMATCH', 'QUERY_ALLOWLIST_REJECTED', 'RUNTIME_EVIDENCE_REJECTED', 'CATALOG_ALLOWLIST_REJECTED'];
        failure(allowed.includes(error.code) ? error.code : 'FIXED_QUERY_OUTPUT_SCHEMA_INVALID', query.queryId);
      }
...
export async function runSealedSnapshot({ request, executionAuthorization, sourceProfile, targetProfile, privateQueryPackManifest, approvedSchemaContract, broker, privateArtifactSink, privateExecutionLedger, packageRoot } = {}) {
  ...
  try {
    ...
    assertReadOnlyConnection(sourceConnection);
    ...
    assertReadOnlyConnection(targetConnection);
    resources.targetConnection = 'open';
    result.targetConnectionState = 'open';
    await sourceConnection.beginReadOnly({ isolationLevel: 'repeatable read', transactionReadOnly: true, searchPath: 'pg_catalog', statementTimeoutMs: 5000, lockTimeoutMs: 1000, idleTimeoutMs: 15000, retryCount: 0 });
    await targetConnection.beginReadOnly({ isolationLevel: 'repeatable read', transactionReadOnly: true, searchPath: 'pg_catalog', statementTimeoutMs: 5000, lockTimeoutMs: 1000, idleTimeoutMs: 15000, retryCount: 0 });
    assertReadOnly(await sourceConnection.attestReadOnly());
    assertReadOnly(await targetConnection.attestReadOnly());
    assertCatalogBindings(await sourceConnection.attestCatalogBindings({ allowlistHash: SECURITY_ALLOWLIST_HASH, catalogBindingsHash: SECURITY_CATALOG_BINDINGS_HASH }));
    assertCatalogBindings(await targetConnection.attestCatalogBindings({ allowlistHash: SECURITY_ALLOWLIST_HASH, catalogBindingsHash: SECURITY_CATALOG_BINDINGS_HASH }));
    ...
    const queryEvidence = [];
    const executionContext = { sourceConnection, targetConnection, sourceRole: sourceProfile.expectedSnapshotRole, targetRole: targetProfile.expectedSnapshotRole, result, evidence: queryEvidence, resources, packageRoot };
    const stage0IdentityRecords = await executePacks({ packIds: ['SOCE-QP01'], ...executionContext });
    assertStage0Attestations(stage0IdentityRecords, sourceProfile, targetProfile, approvedSchemaContract);
    const stage0SchemaRecords = await executePacks({ packIds: ['SOCE-QP02'], ...executionContext });
    ...
    result.stage0 = 'pass';
    const stage1Records = await executePacks({ packIds: ['SOCE-QP03', 'SOCE-QP04', 'SOCE-QP05', 'SOCE-QP06'], ...executionContext });
    assertDomainGates(stage1Records);
    result.stage1 = 'pass';
    assertSanitizedEvidence(queryEvidence);
    assertFinalRuntimeEvidence(await sourceConnection.attestFinalRuntimeEvidence(), sourceProfile.expectedSnapshotRole, FIXED_QUERY_REGISTRY.filter(({ side }) => side === 'source').map(({ queryId }) => queryId));
    assertFinalRuntimeEvidence(await targetConnection.attestFinalRuntimeEvidence(), targetProfile.expectedSnapshotRole, FIXED_QUERY_REGISTRY.filter(({ side }) => side === 'target').map(({ queryId }) => queryId));
