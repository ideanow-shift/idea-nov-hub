...
const packageRelativePath = 'review/store-operations-sealed-snapshot-v1-3-3';
const packageRoot = join(repositoryRoot, packageRelativePath);
const parentAttributesPath = join(repositoryRoot, '.gitattributes');
const parentRule = 'review/store-operations-sealed-snapshot-v1-3-3/** text eol=lf';
const nestedRule = 'review/store-operations-sealed-snapshot-v1-3-3/.gitattributes text eol=lf';
const packageLock = JSON.parse(readFileSync(join(packageRoot, 'execution-package-lock-v1-3-3.json'), 'utf8'));
...
async function assertPackageIntegrity(root) {
  assert.equal(packageLock.artifacts.length, 37);
...
await test('all locked package files retain the 37-artifact and 16-SQL byte contract', () => {
  assert.equal(packageLock.artifacts.length, 37);
...
await test('the v1.3.3 Package Lock verifies all package-level hashes', async () => {
