...
-- Non-disruptive substrate: status reader and SELECT-only resolver stub only.
...
begin
  ...
  if target_count <> 1 then
    raise exception using errcode = '42P01', message = 'CORPORATION_SCOPE_TARGET_NOT_EXACT';
  end if;
  ...
  if owner_oid is not null then
    if not exists (
      ...
      raise exception using errcode = '42501', message = 'CORPORATION_SCOPE_OWNER_ATTRIBUTES_MISMATCH';
    end if;
    if exists (
      select 1 from pg_catalog.pg_auth_members
      where roleid = owner_oid or member = owner_oid
    ) then
      raise exception using errcode = '42501', message = 'CORPORATION_SCOPE_OWNER_MEMBERSHIP_NOT_ZERO';
    end if;
  end if;
  ...
  if exists (
    ...
    raise exception using errcode = '42710', message = 'CORPORATION_SCOPE_PROVIDER_OBJECT_COLLISION';
  end if;
end
$precondition$;
...
returns table (
  ...
  "actorResolverInstalled" boolean,
  "targetResolverInstalled" boolean,
...
as $function$
  select 'CORPORATION_SCOPE_PROVIDER_NOT_READY'::text, false, false, false, false
$function$;

alter function public.read_finance_classification_corporation_scope_provider_status()
  owner to classification_corporation_scope_provider_owner;
...
alter function public.resolve_finance_classification_corporation_scope(uuid)
  owner to classification_corporation_scope_provider_owner;
...
begin
  ...
  if exists (
    ...
    raise exception using errcode = '42501', message = 'CORPORATION_SCOPE_PROVIDER_UNAPPROVED_EXECUTE';
  end if;
end
$postcondition$;
