# Store Operations Sealed Snapshot v1.3.3 Formal Runner Contract Corrective
...
v1.3.3 preserves the v1.3.2 Canonical Operator boundary and synchronizes the
formal runner with the sealed package. QP02 now attests every Canonical
Assignment Foundation relation used by QP04. The package generates an exact
31-field authorization, an approved hash-bound Schema Contract instance, and a
zero-connection execution-ready preflight. Any missing, unknown, or mistyped
authorization field fails closed before credentials or database connections.

v1.3.2 preserves the v1.3.1 Auth boundary and resolves the sales department head
from `employee_organization_assignments`, `organization_assignment_types`, and
`departments` using the effective assignment period. Position and legacy role
labels do not establish the Operator. Auth subject evidence remains owned by
AUTH-01 and is not required by this package.

v1.3.1 removed the `auth.users` aggregate from
