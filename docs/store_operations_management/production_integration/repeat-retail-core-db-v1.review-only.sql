-- REVIEW ONLY. DO NOT EXECUTE IN STAGING OR PRODUCTION.
-- This file deliberately fails before any DDL/DML. A separately approved migration
-- must remove the guard, receive a new filename/SHA, and be reviewed again.

do $$
begin
  raise exception 'REVIEW_ONLY_REPEAT_RETAIL_CORE_DB_V1_EXECUTION_NOT_AUTHORIZED';
end
$$;

-- Proposed change 1: permit the new quantity metric code.
...
values
  ('RETAIL_PURCHASE_CUSTOMER_VISITS', 'POS_RETAIL_PURCHASE_CUSTOMER_COUNT_V1',
   'quantity', '店販購買客数',
   'POS monthly report retail-row customer-count quantity. Zero is a measured zero; NULL is missing.',
   true);

-- Proposed change 2: company-total repeat staging. Existing STORE staging remains unchanged.
create table dbf_ingest.company_repeat_rate_staging_rows (
  ...
  company_id uuid references public.corporations(id) on delete restrict,
  ...
  check (
    (denominator_visit_count = 0 and numerator_cumulative_repeat_count = 0 and exact_rate is null and pos_display_rate = 0)
    or
    (denominator_visit_count > 0 and exact_rate is not null
      and exact_rate between 0 and 1
...
-- Proposed change 3: company-total repeat canonical Fact. No fake store_id exists.
create table public.dbf_company_monthly_repeat_rate_facts (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references public.corporations(id) on delete restrict,
  source_company_no text not null check (btrim(source_company_no) <> ''),
  visit_month date not null check (visit_month = date_trunc('month', visit_month)::date),
  calculation_month date not null,
  horizon_months smallint not null check (horizon_months = 4),
...
create unique index dbf_company_repeat_active_grain_idx
  on public.dbf_company_monthly_repeat_rate_facts
  (company_id, visit_month, customer_segment, definition_version)
  where is_active;

create unique index dbf_company_repeat_version_grain_idx
  on public.dbf_company_monthly_repeat_rate_facts
  (company_id, visit_month, customer_segment, definition_version, version);

create unique index dbf_company_repeat_correction_once_idx
  on public.dbf_company_monthly_repeat_rate_facts (correction_of_fact_id)
  where correction_of_fact_id is not null;

-- Proposed change 4: company-total retail-purchase quantity canonical Fact.
-- SALON rows use the existing public.dbf_store_monthly_metric_facts contract.
-- EC/HQ rows remain only in existing dbf_ingest raw/staging tables.
create table public.dbf_company_monthly_retail_purchase_facts (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references public.corporations(id) on delete restrict,
  source_company_no text not null check (btrim(source_company_no) <> ''),
  ...
  check ((total_customer_visits_for_validation is null or total_customer_visits_for_validation = 0)
    ...
    or total_customer_visits_for_validation > 0
      and retail_purchase_rate_exact_candidate between 0 and 1
...
create unique index dbf_company_retail_purchase_active_grain_idx
  on public.dbf_company_monthly_retail_purchase_facts
  (company_id, fiscal_month, metric_code, definition_version)
  where is_active;

create unique index dbf_company_retail_purchase_version_grain_idx
  on public.dbf_company_monthly_retail_purchase_facts
  (company_id, fiscal_month, metric_code, definition_version, version);

create unique index dbf_company_retail_purchase_correction_once_idx
...
comment on table public.dbf_company_monthly_repeat_rate_facts is
  'Company-total POS repeat cohort facts keyed by customer_segment. Never map RETURNING to SECOND_REPEAT_RATE or SEMI_FIXED to THIRD_REPEAT_RATE. Never add to STORE repeat facts.';
comment on table public.dbf_company_monthly_retail_purchase_facts is
  'Company-total POS retail-purchase customer visit quantities. Never add to SALON totals.';

-- Intentionally omitted pending separate approval:
-- * policies
-- * GRANT to service_role/anon/authenticated
-- * Data API exposure
-- * loader function
-- * any source or business data
