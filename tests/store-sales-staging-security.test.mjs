...
test("sales manager cross-scope store is denied", async () => assert.equal((await request("sales_manager", "synthetic-fc-04")).status, 403));
test("expired session is unauthorized", async () => assert.equal((await request("representative", null, Date.now() - 1)).body.error.code, "SESSION_EXPIRED"));
test("invalid signature is unauthorized", async () => assert.equal((await request("representative", null, Date.now() + 60_000, "bad")).body.error.code, "INVALID_SIGNATURE"));
...
test("synthetic actors cover approved Sprint 2 roles", () => assert.deepEqual(SYNTHETIC_ACTOR_ROLES, ["representative", "sales_manager", "area_manager", "store_manager", "employee"]));
...
test("synthetic stores contain official store labels but no email or corporation data", () => {
  const source = JSON.stringify(SYNTHETIC_STORES);
  assert.doesNotMatch(source, /@|株式会社/i);
});
...
test("synthetic data includes only the current active direct and FC stores", () => {
