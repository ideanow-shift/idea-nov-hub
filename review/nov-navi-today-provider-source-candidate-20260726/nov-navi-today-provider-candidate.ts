...
export async function buildNovNaviTodayEnvelopeForReadyProviders(
  actor: TodayActor,
  ...
  const readyFields = getNovNaviTodayReadyFields(confirmations);
  const settled = await Promise.allSettled(readyFields.map((field) => readAggregate(field)));
  const aggregates: Aggregate = {};
  settled.forEach((result, index) => {
    if (result.status === "fulfilled" && isAggregate(result.value)) {
      aggregates[readyFields[index]] = result.value;
    }
  });
