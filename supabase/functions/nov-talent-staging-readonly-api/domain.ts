...
export function buildCandidateWorkspace(
  ...
  const students = selected.map((row) => Object.freeze({
    ...
    lineIdentifier: canViewContact ? row.line_identifier : null,
    ...
    status: row.line_identifier ? "LINE登録" : "状態未設定",
    statusCode: row.line_identifier ? "LINE_REGISTERED" : null,
    suggestedTargetRecordId: null,
    suggestionCategory: "NONE",
    contactHistory: Object.freeze([]),
  ...
  return Object.freeze({
    ...
    dashboard: Object.freeze({
      ...
      offers: 0,
      withdrawals: 0,
      schoolCount: new Set(students.map((row) => row.school).filter(Boolean)).size,
      fairCount: 0,
      todayActions: 0,
      eventCount: 0,
      selectionHistoryCount: 0,
      availability: Object.freeze({
        candidateCount: true, entries: false, salonTourPlanned: false,
        interviewPlanned: false, offers: false, withdrawals: false,
        schoolCount: true, fairCount: false, todayActions: false,
      }),
    }),
