...
test("Staging browser defaults to formal read-only Store Operations API, not fixture",()=>{assert.match(build,/integrationEndpoint:"\/api\/store-operations"/);assert.doesNotMatch(build,/integrationEndpoint:"\/api\/store-operations-uat-fixture"/);assert.match(server,/pathname==="\/api\/store-operations"/);assert.match(server,/pathname==="\/api\/store-operations-uat-fixture"/);});
