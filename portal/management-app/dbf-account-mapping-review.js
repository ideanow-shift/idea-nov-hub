...
const SAFE_ERRORS = Object.freeze({
  ...
  DECISION_INVALID: "Decisionを選択してください。",
...
function select(doc, values, current, blank = "選択してください") {
  ...
  values.forEach((item) => value.append(option(doc, item, item))); value.value = current || ""; return value;
}
...
export function validateAccountReviewDraft(draft) {
  const errors = {};
  if (!DECISIONS.includes(draft.decision)) errors.decision = "Decisionを選択してください。";
  if (APPROVAL_DECISIONS.has(draft.decision)) {
    if (!String(draft.proposedAccountCode || "").trim()) errors.proposedAccountCode = "Canonical account codeは必須です。";
    if (!String(draft.proposedAccountName || "").trim()) errors.proposedAccountName = "Canonical account nameは必須です。";
    if (!String(draft.accountCategory || "").trim()) errors.accountCategory = "Classificationは必須です。";
    if (!["debit", "credit"].includes(draft.normalBalance)) errors.normalBalance = "Normal balanceを選択してください。";
    if (!["POSTABLE_DETAIL", "DERIVED_SUBTOTAL", "CONTROL_TOTAL", "DISPLAY_ONLY"].includes(draft.rowSemantics)) errors.rowSemantics = "承認可能なRow Semanticsを選択してください。";
    const level = Number(draft.hierarchyLevel);
    if (!Number.isInteger(level) || level < 0 || level > 32) errors.hierarchyLevel = "Hierarchy levelは0から32の整数で入力してください。";
    if (draft.parentCandidateId && !UUID.test(draft.parentCandidateId)) errors.parentCandidateId = "Parent candidate IDの形式が正しくありません。";
  }
...
function reviewRow(doc, item, context) {
  ...
  td("Source account", item.sourceAccountName); td("Candidate code", item.candidateSourceCode);
  ...
  const code = el(doc, "input", "dbf-account-review-input"); code.value = draft.proposedAccountCode; td("Proposed code", field("proposedAccountCode", "Proposed code", code));
  const name = el(doc, "input", "dbf-account-review-input"); name.value = draft.proposedAccountName; td("Proposed name", field("proposedAccountName", "Proposed name", name));
  td("Classification", field("accountCategory", "Classification", select(doc, CATEGORIES[item.statementType] || [], draft.accountCategory)));
  td("Normal balance", field("normalBalance", "Normal balance", select(doc, ["debit", "credit"], draft.normalBalance)));
  td("Row semantics", field("rowSemantics", "Row semantics", select(doc, SEMANTICS, draft.rowSemantics)));
  td("法人 / 将来店舗", `${item.selectedCorporateRowCount} / ${item.futureStoreDetailRowCount}`);
  td("Decision", field("decision", `Decision: ${item.sourceAccountName}`, select(doc, DECISIONS, draft.decision)));
  const action = el(doc, "div", "dbf-account-review-action"); const current = el(doc, "span", "dbf-account-review-current", `現在: ${item.mappingStatus || "UNREVIEWED"}`);
  const save = el(doc, "button", "business-data-action", "保存"); save.type = "button"; const rowStatus = el(doc, "span", "dbf-account-review-row-status"); rowStatus.setAttribute("aria-live", "polite");
  save.addEventListener("click", async () => {
    ...
    context.pending.add(item.candidateId); row.setAttribute("aria-busy", "true"); save.disabled = true; save.textContent = "保存中…"; rowStatus.textContent = "保存しています。"; context.notify(`${item.sourceAccountName}を保存しています。`);
    ...
    context.pending.delete(item.candidateId); row.removeAttribute("aria-busy"); save.disabled = false; save.textContent = "保存";
...
export function createDbfAccountMappingReview(doc, options = {}) {
  ...
  const heading = el(doc, "h3", "", "Account Mapping Review"); heading.tabIndex = -1; panel.append(heading, el(doc, "p", "", "法人会計Actual 2026-06専用。店舗別P/Lは対象外で、Promotionは無効です。"));
  ...
  const filters = el(doc, "div", "dbf-account-review-filters"); const statement = select(doc, ["ALL", "PL", "BS"], "ALL", ""); const state = select(doc, ["ALL", "UNREVIEWED", ...DECISIONS], "ALL", "");
  const search = el(doc, "input", "dbf-account-review-input"); search.type = "search"; search.placeholder = "勘定科目名・コード";
  filters.append(labelledControl(doc, "Statement Type", statement, "dbf-review-filter-statement"), labelledControl(doc, "Decision / Mapping Status", state, "dbf-review-filter-status"), labelledControl(doc, "検索", search, "dbf-review-filter-search"));
  const clear = el(doc, "button", "business-data-secondary-action", "Filterを解除"); clear.type = "button"; filters.append(clear); const count = el(doc, "p", "dbf-account-review-count");
  const filterEmpty = el(doc, "section", "dbf-account-review-state"); filterEmpty.hidden = true; filterEmpty.append(el(doc, "strong", "", "条件に一致する候補はありません。"), el(doc, "p", "", "Filterを解除するか、検索条件を変更してください。"));
  const wrap = el(doc, "div", "dbf-account-review-table-wrap"); wrap.tabIndex = 0; wrap.setAttribute("aria-label", "Account Mapping Review候補一覧。横方向にスクロールできます。");
  const table = el(doc, "table", "dbf-account-review-table"); table.innerHTML = "<caption class=visually-hidden>Account Mapping Review候補</caption><thead><tr><th>Source account</th><th>Candidate code</th><th>Proposed code</th><th>Proposed name</th><th>Classification</th><th>Normal balance</th><th>Row semantics</th><th>法人 / 将来店舗</th><th>Decision</th><th>操作</th></tr></thead>";
  const body = el(doc, "tbody"); table.append(body); wrap.append(table); const promotion = el(doc, "button", "business-data-danger-action business-data-disabled-action", "Promotion disabled（Owner Review完了後も別承認）"); promotion.disabled = true;
  ...
  const render = () => {
    ...
    summary.replaceChildren(metric(doc, "候補", data.summary?.candidates ?? items.length), metric(doc, "承認", data.summary?.approved ?? 0), metric(doc, "編集承認", data.summary?.editAndApproved ?? 0), metric(doc, "除外", data.summary?.excluded ?? 0), metric(doc, "Needs Review", data.summary?.needsReview ?? 0), metric(doc, "未確認", data.summary?.unreviewed ?? 0), metric(doc, "法人P/L", data.summary?.corporatePlRows ?? 0), metric(doc, "法人B/S", data.summary?.corporateBsRows ?? 0));
  ...
  const load = async ({ focusHeading = false } = {}) => {
    ...
    catch (error) { data = null; loading.hidden = true; loaded.hidden = true; panel.removeAttribute("aria-busy"); const safe = safeAccountReviewError(error); alert.hidden = false; alert.replaceChildren(el(doc, "strong", "", safe.message)); if (safe.requestId) alert.append(el(doc, "p", "", `request ID: ${safe.requestId}`)); if (safe.auth) { const launcher = el(doc, "a", "business-data-secondary-action", "NOV HUBへ戻る"); launcher.href = "/"; alert.append(launcher); } const retry = el(doc, "button", "business-data-secondary-action", "再読込"); retry.type = "button"; retry.addEventListener("click", () => void load({ focusHeading: true }).catch(() => {})); alert.append(retry); alert.focus(); throw error; }
  }; context.reload = () => load();
