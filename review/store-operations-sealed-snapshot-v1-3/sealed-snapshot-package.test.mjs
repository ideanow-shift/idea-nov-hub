...
await test('registry fixes 16 actual SQL artifacts, versions, schemas, and byte hashes', () => {
  ...
  for (const query of FIXED_QUERY_REGISTRY) {
    ...
    assert.equal(query.queryVersion, isReadOnlyRoleAttestation ? '1.2.0' : '1.0.0');
    assert.equal(query.expectedOutputSchemaVersion, isReadOnlyRoleAttestation ? '1.2.0' : '1.0.0');
...
await test('SQL character mutation is rejected by the static query hash before run claim or any connection', async () => {
  ...
  try {
    ...
    writeFileSync(join(root, 'execution-package-lock-v1-3.json'), `${JSON.stringify(deriveExecutionPackageLock({ packageRoot: root }), null, 2)}\n`, 'utf8');
    const fixture = makeFixture();
...
process.stdout.write(`RESULT ${passed}/61 PASS\n`);
