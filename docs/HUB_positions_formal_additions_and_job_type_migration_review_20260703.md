...
OS/Core DB指示により、以下をDB更新前レビューとして作成した。
...
2026-07-03に、正式役職4件のINSERTと `レセプション` の `is_active=false` を実行済み。
...
- `レセプション` は `is_active=false`
- `master_change_logs`
  - `employee_attribute.add_required_position`: 4件
  - `employee_attribute.deactivate_job_type_position`: 1件
