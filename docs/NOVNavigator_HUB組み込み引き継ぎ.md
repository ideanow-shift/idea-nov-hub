...
`会長夫人` / `創業者夫人` / `夫人` などの家族関係・敬称ラベルは、役職・職種・雇用形態・権限として扱わないでください。回答ルール、問い合わせルート、LINE WORKS通知先、管理画面権限、HUB Context判定にも使いません。

`レセプション` は役職ではなく職種として扱ってください。職種条件が必要な場合は `employees.job_type_id -> public.job_types.id` を参照します。`一般スタッフ` は役職候補ですが、管理権限ではありません。
