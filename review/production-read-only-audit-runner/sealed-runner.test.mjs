...
await test('sanitizer rejects secret-like or uuid fields', async () => {
  const fixtureUuid = ['12345678', '1234', '1234', '1234', '123456789abc'].join('-');
  assert.throws(() => sanitizeRows([{ opaque_credential: 'x' }], ['opaque_credential'])); assert.equal(maskUuid(fixtureUuid), '12345678…');
});
await test('sql validator rejects write and arbitrary constructs', async () => {
  ...
  assert.equal(validateFixedSql("SELECT 'DELETE' AS policy_command"), true);
});
...
await test('registry contains sixteen validated immutable query ids', async () => {
  assert.equal(QUERY_IDS.length, 16); assert.equal(new Set(QUERY_IDS).size, 16); assert.equal(FIXED_QUERY_REGISTRY.every((query) => validateFixedSql(query.sql)), true);
});
await test('monthly catalog queries accept metadata-only shapes and remain fixed', async () => {
  const db = new FakeAuditConnection({ rowsByQuery: {
    C01_ACCOUNTING_OBJECT_INVENTORY: [{ schema_name: 'accounting', relation_name: 'accounting_versions', relation_kind: 'r', column_name: 'version_number', data_type: 'integer', is_nullable: 'NO', rls_enabled: true, primary_key_count: 1, foreign_key_count: 0, unique_constraint_count: 1, index_count: 2 }],
    C02_CORE_MASTER_INVENTORY: [], C03_GRANT_POLICY_INVENTORY: [], C04_FUNCTION_RPC_INVENTORY: [],
  }});
  const result = await runSealedAudit({ request: request(['C01_ACCOUNTING_OBJECT_INVENTORY', 'C02_CORE_MASTER_INVENTORY', 'C03_GRANT_POLICY_INVENTORY', 'C04_FUNCTION_RPC_INVENTORY']), profile, identityObservation: observation, connection: db });
  assert.equal(result.runStatus, 'complete'); assert.equal(result.queryCount, 4); assert.equal(db.events.at(-2), 'rollback');
});
...
process.stdout.write(`RESULT ${passed}/14 PASS\n`);
