...
import { renderStoreMonthlyBudgetIntake } from "./store-monthly-budget-csv.js";
...
export function buildFinancialReflectionSummary(result) {
  ...
  const corporate = groupStatus.PL_PACKAGE === "LOCAL_PACKAGE_SECTION_READY" || groupStatus.BS_PACKAGE === "LOCAL_PACKAGE_SECTION_READY"
    ? "LOCAL_PREVIEW_ACTIVE"
    : "WAITING_FOR_SOURCE";
  const stores = groupStatus.STORE_PACKAGE === "LOCAL_PACKAGE_SECTION_READY" || groupStatus.PL_PACKAGE === "LOCAL_PACKAGE_SECTION_READY"
    ? "LOCAL_PREVIEW_ACTIVE"
    : "WAITING_FOR_SOURCE";
  const production = "DISABLED_PENDING_CONTRACT";
  return {
  ...
  const entityPreviewRows = [...byEntity.values()].map((entity) => {
    ...
    return {
      ...
      monthlyKpiRows: entity.records.map((record) => ({
        ...
        milbonIdSalesYen: record.milbonIdSalesYen,
...
export function buildFinancialLocalPreview(result) {
  ...
  return {
    ...
    monthlyStoreRows: rows.flatMap((row) => Array.isArray(row.monthlyKpiRows) ? row.monthlyKpiRows : []).slice(0, 10000).map((row) => ({
      ...
      milbonIdSalesYen: row.milbonIdSalesYen,
    ...
    rows: rows.slice(0, 80).map((row) => ({
      ...
      technicalSalesManYen: row.localKpiMetrics?.technicalSalesYen == null ? null : Math.round(Number(row.localKpiMetrics.technicalSalesYen || 0) / 10000),
      productSalesManYen: row.localKpiMetrics?.productSalesYen == null ? null : Math.round(Number(row.localKpiMetrics.productSalesYen || 0) / 10000),
      milbonIdSalesManYen: row.localKpiMetrics?.milbonIdSalesYen == null ? null : Math.round(Number(row.localKpiMetrics.milbonIdSalesYen || 0) / 10000),
      ecSalesManYen: row.localKpiMetrics?.ecSalesYen == null ? null : Math.round(Number(row.localKpiMetrics.ecSalesYen || 0) / 10000),
      storeAnalysisMetricStatus: row.localKpiMetrics ? "SALES_READY_CUSTOMER_REPEAT_MENU_PENDING" : "SALES_BREAKDOWN_PENDING",
...
export function renderFinancialDataIntake(container, hooks = {}) {
  ...
  const storeMonthlyBudget = el(doc, "div", "store-monthly-budget-host");
  storeMonthlyBudget.dataset.storeMonthlyBudgetHost = "true";
  ...
  renderStoreMonthlyBudgetIntake(storeMonthlyBudget, { document: doc });
