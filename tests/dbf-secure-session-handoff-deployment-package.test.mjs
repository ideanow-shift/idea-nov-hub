...
const workflow = read(".github/workflows/dbf-cloud-run-iap-staging-candidate.yml");
...
assert.match(workflow, /environment: dbf-business-data-staging/u);
assert.match(workflow, /dbf-cloud-run-staging-bff-candidate\/Dockerfile/u);
assert.match(workflow, /dbf-staging-ui:\$\{GITHUB_SHA\}/u);
assert.match(workflow, /--no-allow-unauthenticated/u);
assert.match(workflow, /--iap/u);
assert.match(workflow, /NOV_HUB_HANDOFF_EXCHANGE_URL=/u);
assert.doesNotMatch(workflow, /service_account_key|credentials_json|:latest/iu);
