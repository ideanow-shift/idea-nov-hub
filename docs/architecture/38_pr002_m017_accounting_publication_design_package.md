...
A stream-scoped transaction advisory lock prevents concurrent initial/current publications. Every command then takes a request-key advisory lock, rechecks `request_key`, and only afterwards locks the Accounting Version row. The fixed lock order is stream -> request key -> Accounting Version. This makes overlapping identical retries wait and return the committed Publication ID without creating a second release or Audit row. It also serializes concurrent reuse of one request key across different streams without globally serializing unrelated requests. A deferred constraint trigger rechecks at commit that each release has exactly one member, its Version pins match, and the stream has exactly one current published Version.

`request_key` is stable and unique. `request_fingerprint` is a SHA-256 digest of the semantic request tuple: Accounting Version ID, expected content hash, publisher actor, publisher role, reason code, evidence reference, correlation ID, expected prior Publication ID, Canonical Corporation, monthly period and Scenario. Server timestamps and generated Publication IDs are intentionally excluded. A retry is identical only when both the fingerprint and every persisted tuple member match with NULL-safe prior-Publication comparison. Sequential and concurrent identical retries return the existing Publication ID. Any request-key reuse with a different Version, hash, actor, role, reason, evidence, correlation, prior Publication or stream identity fails with `BDF_M017_IDEMPOTENCY_KEY_REUSE_MISMATCH`. `publication_members.accounting_version_id` is unique, so a different request key cannot publish the same Version twice.
