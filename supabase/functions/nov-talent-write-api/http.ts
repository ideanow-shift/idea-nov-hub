...
import {
  ...
  parseWorkforceProcedureCaseStep,
  parseWrite,
  ...
  sanitizeWorkforceProcedureCaseStepResult,
  sanitizeWorkforceProcedureCaseSteps,
...
const WORKFORCE_PROCEDURE_CASE_STEPS='/api/talent/v1/workforce/procedure-cases/steps';
...
export async function handleTalentWrite(req:Request,deps:Deps){const origin=req.headers.get('origin')||'';
 ...
 if(path!==EVENT&&path!==`${EVENT}/invalidate`&&path!==HISTORICAL_REVIEW&&path!==STUDENT_PROFILE&&path!==STAGING_SUPPLEMENT&&path!==WORKFORCE_PROCEDURE_CASES&&path!==WORKFORCE_PROCEDURE_CASE_AUDIT&&path!==WORKFORCE_PROCEDURE_CASE_STEPS)return fail(404,'not_found',origin);
 if(req.method==='OPTIONS'){const h=headers(origin);h.delete('content-type');return new Response(null,{status:204,headers:h});}
 if(req.method!=='POST'&&!(req.method==='GET'&&(path===WORKFORCE_PROCEDURE_CASES||path===WORKFORCE_PROCEDURE_CASE_AUDIT||path===WORKFORCE_PROCEDURE_CASE_STEPS)))return fail(405,'method_not_allowed',origin);
 ...
 if(path===WORKFORCE_PROCEDURE_CASE_STEPS&&req.method==='GET'){
  const caseId=parseWorkforceProcedureCaseAuditQuery(new URL(req.url).searchParams.get('caseId'));
  if(!caseId)return fail(400,'invalid_request',origin);
  try{const result=sanitizeWorkforceProcedureCaseSteps(await deps.rpc(cap,'get_nov_talent_workforce_procedure_case_steps_v1',{
   p_employee_id:cap.actorEmployeeId,p_case_id:caseId
  }));return result?out(200,{ok:true,data:result},origin):fail(503,'not_ready',origin);}catch{return fail(503,'not_ready',origin);}
 }
 ...
 if(path===WORKFORCE_PROCEDURE_CASE_STEPS){const v=parseWorkforceProcedureCaseStep(raw);if(!v)return fail(400,'invalid_request',origin);
  try{const result=sanitizeWorkforceProcedureCaseStepResult(await deps.rpc(cap,'save_nov_talent_workforce_procedure_case_step_v1',{
   p_actor_employee_id:cap.actorEmployeeId,p_case_id:v.caseId,p_step_key:v.stepKey,p_is_completed:v.completed,p_expected_version:v.expectedVersion
  }));return result?out(200,{ok:true,data:result},origin):fail(503,'not_ready',origin);}catch{return fail(503,'not_ready',origin);}}
