...
export function validateDailyWorkflowResponse(value: unknown): { ok: true; value: any } | { ok: false; path: string; rule: string } {
  ...
  if (!Array.isArray(data?.communications) || !Array.isArray(data?.nextActions) || !Array.isArray(data?.assignees)) return bad("data", "arrays");
  if (data.sourceCoverageState === "PREPARING" && (data.communications.length || data.nextActions.length || data.assignees.length)) return bad("data", "preparing_must_be_empty");
  for (const [index, row] of data.communications.entries()) {
    if (!UUID.test(String(row?.id || "")) || !UUID.test(String(row?.candidateId || ""))
      ...
      || typeof row?.awaitingReply !== "boolean" || typeof row?.isEffective !== "boolean"
      || !Number.isInteger(row?.version) || row.version < 1) return bad(`data.communications.${index}`, "shape");
  }
  for (const [index, row] of data.assignees.entries()) {
    if (!UUID.test(String(row?.employeeId || "")) || !String(row?.displayName || "").trim()) return bad(`data.assignees.${index}`, "shape");
  }
  for (const [index, row] of data.nextActions.entries()) {
    if (!UUID.test(String(row?.id || "")) || !UUID.test(String(row?.candidateId || ""))
      || !["OPEN","ON_HOLD","COMPLETED","CANCELLED"].includes(row?.state)
      || !["REGISTERED","UNREGISTERED"].includes(row?.assigneeState)
      || (row.assigneeState === "REGISTERED" && !UUID.test(String(row?.assignedEmployeeId || "")))
      || (row.assigneeState === "UNREGISTERED" && row?.assignedEmployeeId != null)
