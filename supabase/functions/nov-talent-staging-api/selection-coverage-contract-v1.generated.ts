...
export type SelectionCode = "APPLICATION_RECEIVED" | "INTERVIEW_PLANNED" | "INTERVIEW_COMPLETED" | "OFFERED" | "OFFER_ACCEPTED" | "WITHDRAWN" | "REJECTED";
export type SelectionCoverageResponseV1 = { "ok": true; "data": SelectionCoverageDataV1; "meta": SelectionCoverageMetaV1; };
export type SelectionCoverageMetaV1 = { "generatedAt": Timestamp; "requestId": string; "source": string; "version": string; };
export type SelectionCoverageDataV1 = { "selection_coverage_contract_version": "1.0.0"; "sourceCoverageState": "READY" | "PREPARING"; "officialSelectionRows": NullableNonNegativeInteger; "officialUniqueCandidates": NullableNonNegativeInteger; "unlinkedEvidenceTotal": NullableNonNegativeInteger; "datedUnlinkedEvidence": NullableNonNegativeInteger; "undatedUnlinkedEvidence": NullableNonNegativeInteger; "unlinkedUniqueCandidates": NullableNonNegativeInteger; "metrics": ReadonlyArray<SelectionCoverageMetricV1>; };
export type SelectionCoverageMetricV1 = { "code": SelectionCode; "officialRows": NullableNonNegativeInteger; "officialUniqueCandidates": NullableNonNegativeInteger; "unlinkedEvidenceTotal": NullableNonNegativeInteger; "datedUnlinkedEvidence": NullableNonNegativeInteger; "undatedUnlinkedEvidence": NullableNonNegativeInteger; };

type SelectionCoverageJsonSchema = Record<string, any>;
...
function resolveRef(root: SelectionCoverageJsonSchema, ref: string): SelectionCoverageJsonSchema {
  return String(ref).split("/").slice(1).reduce((value, segment) => value?.[segment.replace(/~1/gu, "/").replace(/~0/gu, "~")], root);
}

function validateNode(value: any, node: SelectionCoverageJsonSchema, root: SelectionCoverageJsonSchema, pathName: string): SelectionCoverageValidationResult {
  ...
  if (node.type === "object") {
    ...
    for (const [key, child] of Object.entries(node.properties || {})) if (Object.hasOwn(value, key)) { const result = validateNode(value[key], child as SelectionCoverageJsonSchema, root, pathName + "." + key); if (!result.ok) return result; }
  }
  if (node.type === "array") {
...
export function validateSelectionCoverageResponse(value: any): SelectionCoverageResponseValidationResult {
  const result = validateNode(value, SCHEMA, SCHEMA, "selectionCoverage");
  return result.ok ? { ok: true, value: value as SelectionCoverageResponseV1 } : result;
}
