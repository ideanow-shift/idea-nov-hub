...
alter table public.nov_talent_student_profiles_v1
  add column if not exists offer_date date,
  add column if not exists expected_join_date date,
  add column if not exists planned_store text
    check (planned_store is null or char_length(planned_store) <= 120);

alter table public.nov_talent_student_profile_audit_v1
  drop constraint if exists nov_talent_student_profile_audit_v1_changed_fields_check;
alter table public.nov_talent_student_profile_audit_v1
  add constraint nov_talent_student_profile_audit_v1_changed_fields_check
  check (cardinality(changed_fields) between 1 and 11);

create or replace function public.save_nov_talent_student_profile_v2(
...
begin
  ...
  if not found then
    ...
    insert into public.nov_talent_student_profiles_v1(
      ...
      preferred_store, current_status, next_action_at,
      offer_date, expected_join_date, planned_store, updated_by_employee_id
    ) values (
      v_application_id, p_display_name, p_kana, p_school, p_phone, p_email,
      p_preferred_store, p_current_status, p_next_action_at,
      p_offer_date, p_expected_join_date, p_planned_store, p_actor_employee_id
  ...
  ) values (v_application_id, v_operation, v_changed_fields, p_actor_employee_id, v_version);
...
revoke all on function public.save_nov_talent_student_profile_v2(
  uuid, text, integer, text, text, text, text, text, text, text, date, date, date, text
) from public, anon, authenticated;
grant execute on function public.save_nov_talent_student_profile_v2(
  uuid, text, integer, text, text, text, text, text, text, text, date, date, date, text
) to service_role;
