-- SOCE-QP01-TARGET-READONLY v1.2.0
WITH RECURSIVE
role_paths AS (
  SELECT
...
role_risk AS (
  SELECT
    ...
    (
      ...
      OR role.rolvaliduntil IS NULL
      OR role.rolvaliduntil <= current_timestamp
      OR EXISTS (
        SELECT 1
      ...
      OR EXISTS (
        ...
        WHERE acl.grantee = role.role_oid
...
privilege_counts AS (
  SELECT
    ...
    (SELECT count(*)::integer FROM role_attributes WHERE rolcanlogin) AS login_role_count,
    (SELECT count(*)::integer FROM role_attributes WHERE rolinherit) AS noinherit_violation_count,
    (SELECT count(*)::integer FROM role_attributes WHERE rolconnlimit <> 1) AS connection_limit_violation_count,
    (SELECT count(*)::integer FROM role_attributes WHERE rolvaliduntil IS NULL OR rolvaliduntil <= current_timestamp) AS valid_until_violation_count,
    (SELECT count(*)::integer FROM pg_auth_members AS membership INNER JOIN role_attributes AS role ON role.role_oid = membership.member) AS membership_edge_count,
    ...
    (SELECT count(*)::integer FROM role_attributes AS role CROSS JOIN application_routines AS routine CROSS JOIN LATERAL aclexplode(COALESCE(routine.proacl, acldefault('f', routine.proowner))) AS acl WHERE acl.grantee = role.role_oid AND acl.privilege_type = 'EXECUTE') AS direct_application_routine_execute_count,
...
SELECT
  ...
  counts.login_role_count,
  counts.noinherit_violation_count,
  counts.connection_limit_violation_count,
  counts.valid_until_violation_count,
  counts.membership_edge_count,
  ...
  counts.direct_application_routine_execute_count,
  ...
  (
    ...
    AND counts.reachable_role_count = 1
    AND counts.settable_role_count = 1
    AND counts.inherited_role_count = 1
    ...
    AND counts.login_role_count = 1
    AND counts.noinherit_violation_count = 0
    AND counts.connection_limit_violation_count = 0
    AND counts.valid_until_violation_count = 0
    AND counts.membership_edge_count = 0
    ...
    AND counts.direct_application_routine_execute_count = 0
