...
test('M010 population fixture is deterministic, normalized, and rerunnable', () => {
  ...
  assert.match(populationFixture, /pending_population_id[\s\S]*?expected_item_count[\s\S]*?fixture_date, 21/i);
  assert.match(populationFixture, /pending_store_id, 'pending_review', 'other',[\s\S]*?false, 'pending_review'/i);
  assert.doesNotMatch(populationFixture, /pending_store_id, 'official_operating'[\s\S]*?'pending_review'/i);
  assert.match(populationFixture, /pending_population_id, store_ids\[i\], 'official_operating'[\s\S]*?true, 'approved'/i);
  assert.match(populationFixture, /pending_count <> 1/);
  assert.match(populationFixture, /sqlerrm = 'BDF_POPULATION_PUBLICATION_GATE_FAILED'/i);
  assert.match(populationFixture, /BDF_TEST_PENDING_REVIEW_REJECTED pending_review_count=1/);
