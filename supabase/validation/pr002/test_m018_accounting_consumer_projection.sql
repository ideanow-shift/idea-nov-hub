-- Synthetic M018 Consumer Projection contract test. Entire fixture is rolled back.
-- It includes the M017 publication fixture to prove the current-baseline boundary.
begin;
...
insert into accounting.account_statement_mappings(
  statement_mapping_version_id,account_id,account_version_id,version_no,statement_type,
  statement_section,statement_line,display_order,aggregation_behavior,contribution_sign,
  effective_from,effective_to,status,mapping_contract_version,content_digest,recorded_by
) values
  ('18000000-0000-4000-8000-000000000201','17000000-0000-4000-8000-000000000200',
   '17000000-0000-4000-8000-000000000201',1,'pl','operating_expense','operating_expense',10,
   'add',1,'2026-01-01','2027-01-01','active','mapping-v1',repeat('1',64),'audit:m018'),
  ('18000000-0000-4000-8000-000000000203','17000000-0000-4000-8000-000000000202',
   '17000000-0000-4000-8000-000000000203',1,'pl','revenue','revenue',20,
   'subtract',-1,'2026-01-01','2027-01-01','active','mapping-v1',repeat('2',64),'audit:m018');
...
insert into accounting.journal_lines(
  ...
  ('18000000-0000-4000-8000-000000000435','17000000-0000-4000-8000-000000000430',
   '17000000-0000-4000-8000-000000000310','m017_revision',repeat('a',64),3,repeat('b',64),3,
   '17000000-0000-4000-8000-000000000200','17000000-0000-4000-8000-000000000201',
   '17000000-0000-4000-8000-000000000100','17000000-0000-4000-8000-000000000101',
   'corporation','period_flow','zero','planning-v2',repeat('c',64),'audit:m018'),
  ('18000000-0000-4000-8000-000000000436','17000000-0000-4000-8000-000000000430',
   '17000000-0000-4000-8000-000000000310','m017_revision',repeat('d',64),4,repeat('e',64),4,
   '17000000-0000-4000-8000-000000000200','17000000-0000-4000-8000-000000000201',
   '17000000-0000-4000-8000-000000000100','17000000-0000-4000-8000-000000000101',
   'corporation','period_flow','not_applicable','planning-v2',repeat('f',64),'audit:m018');
insert into accounting.accounting_facts(
  accounting_fact_id,journal_line_id,journal_entry_id,accounting_version_id,corporation_id,
  organization_scope_type,accounting_period,account_id,measure_type,amount,currency_code,tax_basis,
  value_status,attribution_status,derivation_status,source_line_digest,recorded_by
  ...
  ('18000000-0000-4000-8000-000000000437','18000000-0000-4000-8000-000000000435',
   '17000000-0000-4000-8000-000000000430','17000000-0000-4000-8000-000000000310',
   '17000000-0000-4000-8000-000000000100','corporation','2026-05-01',
   '17000000-0000-4000-8000-000000000200','period_flow',0,'JPY','exclusive','zero',
   'directly_attributed','planning',repeat('b',64),'audit:m018'),
  ('18000000-0000-4000-8000-000000000438','18000000-0000-4000-8000-000000000436',
   '17000000-0000-4000-8000-000000000430','17000000-0000-4000-8000-000000000310',
   '17000000-0000-4000-8000-000000000100','corporation','2026-05-01',
   '17000000-0000-4000-8000-000000000200','period_flow',null,'JPY','exclusive','not_applicable',
   'not_applicable','planning',repeat('e',64),'audit:m018');
update accounting.accounting_versions set status='validating',validating_at=statement_timestamp(),
...
declare n integer;
begin
  select count(*) into n from projection.accounting_publication_status_v1;
  if n<>1 then raise exception 'M018_DUPLICATE_CURRENT_PROJECTION %',n; end if;
  raise notice 'M018_EXPECTED SUPERSEDED_EXCLUDED';

  if exists (select 1 from projection.accounting_publication_status_v1
    where accounting_version_id in (
      '17000000-0000-4000-8000-000000000300',
      '17000000-0000-4000-8000-000000000320',
      '17000000-0000-4000-8000-000000000321')) then
    raise exception 'M018_UNPUBLISHED_VERSION_EXPOSED';
  end if;
  raise notice 'M018_EXPECTED APPROVED_NOT_PUBLISHED_EXCLUDED';
  raise notice 'M018_EXPECTED NO_PUBLICATION_NO_PROJECTION';

  if (select count(*) from projection.accounting_corporation_pl_v1)<>4 then
    raise exception 'M018_PL_ROW_COUNT';
  end if;
  if not exists (select 1 from projection.accounting_corporation_pl_v1
    where value_status='zero' and amount=0) then
    raise exception 'M018_FORMAL_ZERO_LOST';
  end if;
  raise notice 'M018_EXPECTED FORMAL_ZERO_PRESERVED';
  if not exists (select 1 from projection.accounting_corporation_pl_v1
    where value_status='not_applicable' and amount is null) then
    raise exception 'M018_NULL_CHANGED_TO_ZERO';
  end if;
  raise notice 'M018_EXPECTED NULL_NOT_ZERO';

  if exists (select 1 from projection.accounting_corporation_pl_v1
    where scenario_type<>'budget' or measure_type<>'period_flow'
       or organization_scope_type<>'corporation') then
    raise exception 'M018_WRONG_PROJECTION_CONTRACT';
  end if;
  if (select count(*) from projection.accounting_corporation_bs_v1)<>0
    or (select count(*) from projection.accounting_store_profit_v1)<>0
    or (select count(*) from projection.accounting_cash_flow_v1)<>0 then
    raise exception 'M018_SCOPE_LEAK';
  end if;
end
$projection$;

select pg_temp.expect_failure('CONSUMER_VIEW_DML',$q$
  update projection.accounting_corporation_pl_v1 set amount=999
$q$,'cannot update view');

do $access$
begin
  begin
    set local role anon;
    perform 1 from accounting.accounting_facts limit 1;
    raise exception 'M018_RAW_FACT_MISPASS';
  exception when insufficient_privilege then
    raise notice 'M018_EXPECTED RAW_FACT_ACCESS';
  end;
  reset role;
  begin
    set local role authenticated;
    perform 1 from projection.accounting_corporation_pl_v1 limit 1;
    raise exception 'M018_UNAUTHORIZED_CONSUMER_MISPASS';
  exception when insufficient_privilege then
    raise notice 'M018_EXPECTED UNAUTHORIZED_CONSUMER';
  end;
  reset role;
end
$access$;
