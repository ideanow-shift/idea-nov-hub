export function createStoreOperationsHandoffRpcStore(callRpc){return Object.freeze({
  async insert(row){const result=await callRpc("store_operations_handoff_issue_v1",{
    ...
    p_hub_session_expires_at:new Date(row.hubSessionExpiresAt).toISOString(),p_contract:row.contract,p_target:row.target,p_target_origin:row.targetOrigin,
    p_callback_path:row.callbackPath,p_audience:row.audience,p_state_hash:row.stateHash,p_nonce_hash:row.nonceHash,p_code_challenge:row.codeChallenge,p_request_id:row.requestId,
  ...
  async consumeAtomic(match){const result=await callRpc("store_operations_handoff_consume_v1",{
    p_code_hash:match.codeHash,p_state_hash:match.stateHash,p_nonce_hash:match.nonceHash,p_code_challenge:match.codeChallenge,p_contract:match.contract,p_target:match.target,
    p_target_origin:match.targetOrigin,p_callback_path:match.callbackPath,p_audience:match.audience,p_consumed_at:new Date(match.now).toISOString(),
