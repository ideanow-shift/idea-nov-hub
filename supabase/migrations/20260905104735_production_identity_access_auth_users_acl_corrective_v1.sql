-- Corrective-only migration: keep resolver and ledger guard SECURITY INVOKER.
-- The narrowly scoped helper is the sole auth.users privilege boundary and returns no identity data.
begin;

create function identity_access.auth_user_active_v1(p_auth_user_id uuid)
returns boolean
language sql
stable
security definer
...
alter function identity_access.auth_user_active_v1(uuid) owner to postgres;
revoke all on function identity_access.auth_user_active_v1(uuid) from public,anon,authenticated,service_role;
grant execute on function identity_access.auth_user_active_v1(uuid) to service_role;

create or replace function identity_access.guard_decision_v1() returns trigger
...
begin
  ...
  if tg_table_name='auth01_binding_decisions' then
    ...
    if not identity_access.auth_user_active_v1(new.auth_user_id) then
...
create or replace function public.store_operations_production_access_v1(p_subject_digest text)
...
begin
  ...
  select a.* into binding from identity_access.auth01_latest a
    ...
    where a.subject_digest=p_subject_digest
      and identity_access.auth_user_active_v1(a.auth_user_id) and a.provider='nov_hub' and a.issuer='nov_hub_production'
...
revoke all on function identity_access.guard_decision_v1() from public,anon,authenticated;
...
comment on function identity_access.auth_user_active_v1(uuid) is
 'Service-only boolean auth anchor activity check. No auth row, email, metadata, subject, or token is returned.';
comment on function public.store_operations_production_access_v1(text) is
 'Server-only AUTH-01/M019 Production port. Resolver remains SECURITY INVOKER; auth.users access is isolated in a service-only boolean helper.';
commit;
